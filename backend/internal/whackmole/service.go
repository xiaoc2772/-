...
const (
	...
	submitEarlyGraceMillis = int64(5_000)
	maxRetryableTxRetries  = 12
	defaultRecordsListSize = 10
)

var ErrUnavailable = errors.New("whack mole database unavailable")
...
func (service *Service) Status(ctx context.Context, user auth.User) (StatusData, error) {
	...
	err := service.withRetryableTx(ctx, func(tx pgx.Tx) error {
		...
		if active != nil {
			view := BuildSessionView(*active, time.Now())
			activeView = &view
		}
...
func (service *Service) Sync(ctx context.Context, user auth.User) (*SessionView, error) {
	if service.db == nil {
		return nil, ErrUnavailable
	}

	var output *SessionView
	err := service.withRetryableTx(ctx, func(tx pgx.Tx) error {
		if err := ensureUser(ctx, tx, user); err != nil {
			return err
		}
...
func (service *Service) Submit(ctx context.Context, user auth.User, input SubmitInput) (SubmitResult, error) {
	...
	err := service.withRetryableTx(ctx, func(tx pgx.Tx) error {
		...
		if candidateEvents == nil {
			candidateEvents = session.Events
		}
		candidateEvents = NormalizeEvents(candidateEvents, difficulty)
		if ok, message := ValidateEventsRate(candidateEvents); !ok {
			output = SubmitResult{Success: false, Message: message}
			return nil
		}
		serverDuration := millis(now) - session.StartedAt
		if serverDuration < config.DurationMs-submitEarlyGraceMillis {
			output = SubmitResult{Success: false, Message: "游戏尚未结束"}
			return nil
		}

		scored := ScoreEvents(session.Seed, candidateEvents, difficulty)
		pointReward := CalculatePointReward(scored.Score, difficulty)
		dailyLimit, err := systemconfig.DailyPointsLimit(ctx, tx)
		if err != nil {
			return err
		}
		pointsEarned, dailyEarned, err := addGamePointsWithLimit(ctx, tx, user, pointReward, dailyLimit, fmt.Sprintf("打地鼠（%s）得分 %d，福利积分 %d", config.Label, scored.Score, pointReward))
		if err != nil {
			return err
		}
		record := Record{
			...
			Hits:         scored.Stats.Hits,
			GoldenHits:   scored.Stats.GoldenHits,
			Misses:       scored.Stats.Misses,
			Bombs:        scored.Stats.Bombs,
			MaxCombo:     scored.Stats.MaxCombo,
		...
		if err := incrementDailyStats(ctx, tx, user.ID, scored.Score, dailyEarned, now); err != nil {
			return err
		}
		if err := deleteSessionAndActive(ctx, tx, user.ID, session.ID); err != nil {
			return err
...
func BuildSessionView(session Session, now time.Time) SessionView {
	difficulty := NormalizeDifficulty(session.Difficulty)
	config := DifficultyConfigFor(difficulty)
	events := NormalizeEvents(session.Events, difficulty)
	elapsedMs := maxInt64(0, millis(now)-session.StartedAt)
	boardElapsedMs := minInt64(elapsedMs, config.DurationMs-1)
	scored := ScoreEvents(session.Seed, events, difficulty)
	board := EmptyBoard()
	if elapsedMs < config.DurationMs {
		board = GetBoard(session.Seed, boardElapsedMs, difficulty)
	}
	return SessionView{
		...
		Difficulty:  difficulty,
		Board:       board,
		BoardTick:   GetTickIndex(boardElapsedMs, difficulty),
		TimeLeftMs:  maxInt64(0, config.DurationMs-elapsedMs),
		Score:       scored.Score,
		Combo:       scored.Combo,
